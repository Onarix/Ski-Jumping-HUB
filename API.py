...
def getResults():
    ...
    year = today.year
    url = (
        "https://www.fis-ski.com/DB/general/statistics.html?statistictype=positions&positionstype=position&offset=50&sectorcode=JP&seasoncode="
        + str(year + 1)
        + "&categorycode=WC&gendercode=M&competitornationcode=&place=&nationcode=&position=4&disciplinecode="
    )
    ...
    comps = soup.find_all(
        "a",
        class_="bb-xs pb-xs-1_1 g-xs-11 g-sm-6 g-md-4 g-lg-4 justify-left bold",
        href=True,
    )
    dates_soup = soup.find_all(
        "a", class_="bb-xs pb-xs-1_1 pl-xs-1 g-xs-6 g-sm-3 g-md-2 g-lg-2 justify-left"
    )
